...
public:
    ...
    /**
     * @brief Initializer function expecting a ProtoBuf class that has to be written to a file descriptor.
     *
     * @param toWrite A ProtoBuf generated class that contains the SerializeToString member
     * function.
     * @throw If serializing fails or if it has already been initialized a std::runtime_error will be thrown.
     */
    void init(T& toWrite)
    {
    ...
    bool write(const ipc::FileDescriptor& fd)
    {
        if (!_sizeWritten) {
            ...
            if (send(fd.getFd(), &size, sizeof(size), 0) == -1)
            ...
            isize       bytesSent = send(fd.getFd(), data, writeSize, 0);
            if (bytesSent == -1)
